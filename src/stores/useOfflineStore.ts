...
export const useOfflineStore = create<OfflineState>((set, get) => ({
  ...
  prepare: async (force = false) => {
    ...
    try {
      ...
      const covers = (books?.data || []).filter(item => item.active && !item.deleted_at).map(item => item.cover_url).filter((url): url is string => Boolean(url))
      const results = await cacheOfflineAssets(logos, photos, covers)
      ...
      set({ readiness, progress: failed.length ? `Pacote essencial pronto. ${failed.length} imagem(ns) opcional(is) não foram salvas; o aplicativo usará uma capa padrão.` : 'Tudo pronto para uso offline.' })
    } catch (error) {
