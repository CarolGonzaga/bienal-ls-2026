...
const emptyPresence = { presence_date: '', start_time: '', end_time: '', exhibitor_id: '', stand_code: '', notes: '', guaranteed: true }
...
export default function AuthorContentRequests({ authorId, notice, agendaOnly = false, localScenario = null }) {
  ...
  const load = async () => {
    ...
    const [exhibitorResult, linksResult, requestResult] = await Promise.all([
      ...
      supabase.from('author_change_requests').select('id,request_type,payload,status,created_at').eq('author_id', authorId).in('request_type', ['presence','book','availability','autograph']).order('created_at', { ascending: false })
    ])
  ...
  const submit = async event => {
    ...
    notice('Informação enviada para aprovação administrativa.');
    if (kind === 'presence') setPresence(emptyPresence); if (kind === 'book') setBook(emptyBook); if (kind === 'availability') setAvailability(emptyAvailability); if (kind === 'autograph') setAutograph(emptyAutograph)
    void load()
  }
  const options = agendaOnly ? [['presence','Presença',MapPin],['autograph','Autógrafo',CalendarDays]] : [['presence','Presença',MapPin],['book','Livro',BookOpen],['availability','Venda',Store],['autograph','Autógrafo',CalendarDays]]
  const activeKind = agendaOnly && !['presence', 'autograph'].includes(kind) ? 'presence' : kind
  return <section className="auth-card mt-5 rounded-3xl border p-5"><div><h2 className="text-lg font-black">{agendaOnly ? 'Minha agenda na Bienal' : 'Presença, livros e agenda'}</h2><p className="mt-1 text-xs opacity-70">Toda informação é enviada para revisão antes de aparecer no Mapa{agendaOnly ? ' e na programação.' : ' e no Passaporte.'}</p></div><div className={`mt-4 grid gap-2 ${agendaOnly ? 'sm:grid-cols-2' : 'sm:grid-cols-4'}`}>{options.map(([id,label,Icon]) => <button type="button" key={id} onClick={() => setKind(id)} className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-black ${activeKind === id ? 'bg-[#d43276] text-white' : 'auth-input'}`}><Icon className="h-4 w-4"/>{label}</button>)}</div><form className="mt-4 grid gap-3" onSubmit={submit}>
