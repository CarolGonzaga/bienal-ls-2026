...
const initialForm = { book_name: '', author: '', publisher: '', stand_code: '', notes: '', author_name: '', books: '', event_date: '', start_time: '', location_text: '', official_link: '', exhibitor_name: '', location: '', entity_type: '', description: '', wrong_info: '', correct_info: '', source: '', submitter_name: '', submitter_contact: '' }
    ...
    <section className="contribution-sheet flex max-h-[94dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[2rem] border sm:max-h-[90dvh] sm:rounded-[2rem]">
      ...
      <div className="overflow-y-auto px-5 pb-6 sm:px-7">
        {result ? <div className="flex min-h-64 flex-col items-center justify-center gap-4 text-center"><CheckCircle2 className="h-12 w-12 text-emerald-500"/><p className="contribution-title max-w-md font-bold">{result}</p><button onClick={resetAndClose} className="contribution-submit rounded-xl px-6 py-3 font-black text-white">Concluir</button></div> : !type ? <div className="flex flex-col gap-3 pb-2">
          ...
          {type === 'sapphic_book' && <>{field('book_name', 'Nome do livro', true, 'Ex: Presas')}{field('author', 'Autora', true, 'Ex: Rebecca Nobre')}{field('publisher', 'Editora', false, 'Ex: Galera Record')}{standField}<label className="contribution-label flex flex-col gap-1.5 text-xs font-black">Observação<textarea value={form.notes} onChange={event => update('notes', event.target.value)} className="contribution-input min-h-24 rounded-xl border p-4 text-sm outline-none"/></label>{commonFields}</>}
