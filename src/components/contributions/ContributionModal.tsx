...
export const ContributionModal: React.FC<{ open: boolean; onClose: () => void; user: User; exhibitors: Exhibitor[] }> = ({ open, onClose, user, exhibitors }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'autograph_session') {
        if (form.event_date < BIENAL_START_DATE || form.event_date > BIENAL_END_DATE) throw new Error('A data deve estar entre 04/09/2026 e 13/09/2026.')
        const timeError = validateEventTime(form.event_date, form.start_time, '')
  ...
  const field = (key: keyof typeof form, label: string, required = false, placeholder = '', typeName = 'text', inputProps: React.InputHTMLAttributes<HTMLInputElement> = {}) => <label className="contribution-label flex flex-col gap-1.5 text-xs font-black">{label}{required && ' *'}<input required={required} type={typeName} value={form[key]} onChange={event => update(key, event.target.value)} placeholder={placeholder} className="contribution-input rounded-xl border px-4 py-3 text-sm outline-none" {...inputProps}/></label>
