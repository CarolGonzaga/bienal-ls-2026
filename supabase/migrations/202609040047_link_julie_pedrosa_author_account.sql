-- Libera o painel de autoras para Julie Pedrosa.
-- O UID é a identidade canônica; o e-mail é obtido diretamente de auth.users
-- porque o valor recebido ("escritorajuliepedrosa") não é um endereço válido.
do $$
declare
  target_name constant text := 'Julie Pedrosa';
  target_source_id constant text := 'julie-pedrosa';
  target_user_id constant uuid := 'a5892839-eeec-4e1f-ab17-342084464ca2';
...
begin
  ...
  if auth_email is null then
    raise exception 'Conta do Auth não encontrada ou sem e-mail para o UID: %', target_user_id;
  end if;
  ...
  values (target_user_id, auth_email, target_name, 'author', now())
  ...
  if not exists (
    ...
    where account.author_id = target_author_id
      ...
      and lower(trim(profile.email)) = auth_email
  ) then
    raise exception 'O vínculo de Julie Pedrosa não passou pela verificação final';
  end if;

  raise notice 'Acesso ao painel confirmado para Julie Pedrosa (%) com o e-mail cadastrado no Auth.', target_user_id;
end;
$$;
