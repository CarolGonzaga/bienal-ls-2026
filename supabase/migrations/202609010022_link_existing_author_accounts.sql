...
begin
  if jsonb_array_length(author_links) <> 23 then
    raise exception 'A lista de vínculos deve conter exatamente 23 autoras únicas';
  end if;
  ...
  if verified_link_count <> 23 then
    raise exception 'Verificação final incompleta: % de 23 vínculos confirmados', verified_link_count;
  end if;

  raise notice '23 vínculos confirmados; % entidade(s) de autora criada(s)', created_author_count;
end;
$$;
