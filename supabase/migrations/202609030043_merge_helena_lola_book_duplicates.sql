-- Consolida duplicatas de Helena Vieira e Lola Chambrett sem perder capa,
-- vínculos editoriais, listas das leitoras ou marcações de autógrafo.
...
select public.merge_passport_book_duplicate(
  'comoseduziranovata',
  'helenavieira',
  'Como Seduzir A Novata',
  'Helena Vieira',
  'Vênus',
  'Romance sáfico',
  array['autora independente', 'hot +18', 'romance sáfico']::text[],
  'H85',
  false
);
...
select public.merge_passport_book_duplicate(
  ...
  'P.S',
  'Comédia Romântica',
  array['romance', 'nacional', 'comédia romântica', '18+']::text[],
  'H85',
  true
);
...
create unique index if not exists books_como_seduzir_a_novata_helena_unique
on public.books ((1))
where deleted_at is null
  and regexp_replace(translate(lower(trim(title)), 'áàâãäéèêëíìîïóòôõöúùûüç', 'aaaaaeeeeiiiiooooouuuuc'), '[^a-z0-9]+', '', 'g') = 'comoseduziranovata'
  and regexp_replace(translate(lower(trim(author_name)), 'áàâãäéèêëíìîïóòôõöúùûüç', 'aaaaaeeeeiiiiooooouuuuc'), '[^a-z0-9]+', '', 'g') = 'helenavieira';

create unique index if not exists books_o_que_os_olhos_nao_veem_lola_unique
on public.books ((1))
where deleted_at is null
  and regexp_replace(translate(lower(trim(title)), 'áàâãäéèêëíìîïóòôõöúùûüç', 'aaaaaeeeeiiiiooooouuuuc'), '[^a-z0-9]+', '', 'g') = 'oqueosolhosnaoveem'
  and regexp_replace(translate(lower(trim(author_name)), 'áàâãäéèêëíìîïóòôõöúùûüç', 'aaaaaeeeeiiiiooooouuuuc'), '[^a-z0-9]+', '', 'g') = 'lolachambrett';
